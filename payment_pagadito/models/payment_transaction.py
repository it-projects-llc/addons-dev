...
class TransactionPagadito(models.Model):
    ...
    def _pagadito_form_validate(self, data):
        ...
        # get_status
        status_data = self.acquirer_id._pagadito_call(pagadito.OP_GET_STATUS, {
            'token': token,
        ...
        if status in [pagadito.STATUS_COMPLETED]:
            ...
            _logger.info('Received notification for Pagadito payment %s: set as pending' % (self.reference))
            vals.update(state='pending', state_message=status)
            return self.write(vals)
        else:
            error = 'Received unrecognized status for Pagadito payment %s: %s, set as error' % (self.reference, status)
            _logger.info(error)
