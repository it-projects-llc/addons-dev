...
class WebKitParser(report_sxw):
    ...
    def generate_pdf(self, comm_path, report_xml, header, footer, html_list, webkit_header=False):
        ...
        fd, out_filename = tempfile.mkstemp(suffix=".pdf",
                                            prefix="webkit.tmp.")
        #print "===fd, out_filename===",fd, out_filename
        ...
        try:
            #print "===stderr_fd===",command,stderr_fd
    ...
    def create_single_pdf(self, cursor, uid, ids, data, report_xml, context=None):
        ...
        with registry.cursor() as cr:
            ...
            #print "===head_mako_tpl===",head_mako_tpl.render(dict(parser_instance.localcontext, _debug=False))
            try :
    ...
    def create(self, cursor, uid, ids, data, context=None):
        ...
        registry = odoo.registry(cursor.dbname)
        #ir_obj = pool['ir.actions.report.xml']
        with registry.cursor() as cr:
            env = api.Environment(cr, SUPERUSER_ID, {})
            #print "===self.name[7:]===",self.name
            report_xml_ids = env['ir.actions.report.xml'].with_context(context=context).search([('report_name', '=', self.name[7:])])
            #report_xml_ids = ir_obj.search(cursor, uid, [('report_name', '=', self.name[7:])], context=context)
            #print "==report_xml_ids=",report_xml_ids
            if report_xml_ids:
                report_xml = report_xml_ids#env['ir.actions.report.xml'].with_context(context=context).browse(report_xml_ids[0])
            else:
                return super(WebKitParser, self).create(cursor, uid, ids, data, context)
            #print "===report_xml===",report_xml,self.header,report_xml.header
            setattr(report_xml, 'use_global_header', self.header if report_xml.header else False)
    
            if report_xml.report_type != 'webkit':
                return super(WebKitParser, self).create(cursor, uid, ids, data, context)
            #print "===uid, ids, data, report_xml==",uid, ids, data, report_xml
