...
models.load_fields('event.registration',['signed_terms']);
...
pos_event.AttendeeListScreenWidget.include({
    ...
    action_show_attendeed_button: function(){
        ...
        // for some reason on loading event_id = [id, name], after reload attendees event_id = id
        // i think a function such as get_event_id is necessary
        // probably make a class for attendees like for products
        var attendee = this.current_attendee;
        var event_id = attendee && attendee.event_id[0] || attendee.event_id;
        var mandatory_esign = event_id && this.pos.event_set.event_ids[event_id].mandatory_esign
        if (!mandatory_esign || (mandatory_esign && attendee.signed_terms)) {
            this._super();
        } else {
