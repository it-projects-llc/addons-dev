...
class PosWeChatMiniProgramOrder(models.Model):
    ...
    def create_from_miniprogram_ui(self, lines, create_vals):
        ...
        """
        _logger.debug('Create Order from WeChat mini-program: lines - %s, create values - %s', lines, create_vals)

        if self.env.user.number_verified is False:
            raise UserError(_("Mobile phone number not specified for User: %s (id: %s)")
    ...
    def _update_order_state(self):
        self.ensure_one()
        self.write({
            'state': self.wechat_order_id.state
        })
