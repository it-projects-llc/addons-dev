...
class FleetRentalCreateInvoiceWizard(models.TransientModel):
    ...
    @api.multi
    def _create_invoice(self, document_rent, amount):
        ...
        invoice = inv_obj.create({
            'name': document_rent.name,
            'origin': document_rent.name,
            'type': 'out_invoice',
            'reference': False,
            'account_id': document_rent.partner_id.property_account_receivable_id.id,
            'partner_id': document_rent.partner_id.id,
    ...
    def create_invoices(self):
        documents_rent = self.env['fleet_rental.document_rent'].browse(self._context.get('active_ids', []))
        ...
        for document_rent in documents_rent:
            ...
            self._create_invoice(document_rent, amount)

        if self._context.get('open_invoices', False):
            return documents_rent.action_view_invoice()
