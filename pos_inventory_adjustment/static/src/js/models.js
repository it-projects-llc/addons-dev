...
odoo.define('pos_inventory_adjustment.models', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        get_inventory_stages: function (ids) {
            ...
            if (ids) {
                return rpc.query({
                    model: 'stock.inventory.stage',
                    method: 'search_read',
                    args: [[['id', 'in', ids]], ['name', 'inventory_id', 'user_id', 'line_ids']],
                }).then(function (inv_stages) {
                    return inv_stages;
                }, function (err) {
                    console.log(err);
                });
            ...
            return rpc.query({
                    model: 'stock.inventory.stage',
                    method: 'search_read',
                    args: [[['state', '=', 'open']], ['name', 'inventory_id', 'user_id', 'line_ids']],
                }).then(function (inv_stages) {
                return inv_stages;
            }, function (err) {
                console.log(err);
        ...
        get_inventory_stage_lines: function (ids) {
            var self = this;
            return rpc.query({
                model: 'stock.inventory.stage.line',
                method: 'read',
                args: [ids],
            }).then(function (lines) {
                return lines;
        ...
        send_inventory_stage: function() {
            ...
            return rpc.query({
                model: 'stock.inventory.stage',
                method: 'update_stage_from_ui',
                args: [order.inventory_adjustment_stage_id, order_to_send],
            });
        },
    });
